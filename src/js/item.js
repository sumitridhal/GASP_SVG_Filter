...
export class Item {
  constructor(el) {
    ...
    this.DOM.feDisplacementMap =
      this.DOM.SVGFilter.querySelector('feDisplacementMap');

    // (turbulence) baseFrequency or (displacementMap) scale current value
    this.primitiveValues =
      this.filterType === 'turbulence' ? { baseFrequency: 0 } : { scale: 0 };

    this.DOM.caption = this.DOM.thumb.querySelector('.thumb__caption');
    this.DOM.captionTitle = this.DOM.caption.querySelector(
      '.thumb__caption-title'
    );
    this.DOM.captionLink = this.DOM.caption.querySelector(
      '.thumb__caption-link'
    );
  ...
  createHoverTimeline() {
    ...
    this.tl
      ...
      .to(
        [this.DOM.captionTitle, this.DOM.captionLink],
        {
          y: 0,
      ...
      .to(
        [this.DOM.metaCounter, this.DOM.metaTitle, this.DOM.metaDetail],
        {
          duration: 0.1,
          x: (i) => (i % 2 == 0 ? '-5%' : '5%'),
          opacity: 0,
